...
const [isClick,setIsClick]=useState(false);
  ...
  return (
    <div className='SquareSplit_game'>
      <h1>BOX SPLITING</h1>
      <div className='square_board' onClick={handleClick}>{isClick?<CreateSquare/>:""}
      </div>
    </div>
  )
}
...
const CreateSquare=()=>{
 const [square,setSqaure]=useState([true,true,true,true])
  return(
<div className='Create_Square'>{square.map((val,index)=><Square val={val} index={index} key={index}/>)}</div>
  )
}
